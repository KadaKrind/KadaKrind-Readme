# KadaKrind-Readme
Bio for me Rodrigo Porcel, Pls read for more information
# Hi, I'm Rodrigo Porcel (KadaKrind)

Virtual assistant with skills in **web development**, **SEO**, and **digital marketing**.

## Skills
- WordPress development and customization
- HTML, CSS, JavaScript, python, C#, c++ 
- SEO strategy and on-page optimization
- Email communication and automation (e.g., using scripts, tools)
- Data scraping and web research
- Content writing and editing
- Project management and marketing coordination
- Basic Python scripting for automation
- CRM tools familiarity
- Fluent English and Spanish communication

## Contact
- LinkedIn: https://www.linkedin.com/in/rodrigo-miguel-p-2443b213a
- Email: porcel1232002@gmail.com
## Projects
You can find my professional portfolio projects inside the [portafolio](./portafolio) folder
